...
if __name__ == '__main__':
	...
	theta = np.array([0.0 , 0.71483946 , 0.04682921 , 0.18070403 , 0.13391187])
	theta = np.zeros(5)
	...
	for i in range(10):
		temp = 0
		print("Theta",theta)
		theta0 = gradient_descent(theta, X, y, m, learning_rate)
		improvement = theta - theta0
		if np.dot(improvement,improvement) <= 0.00000001:
			temp = 1
		s = 0
		theta = theta0	
		for j in range(m):
	...
	#Training RMSE :  1008161.6685234178
	...
	#REGULARISATION
	regularised_weight = [0.001, 0.01, 0.1, 1, 10, 100]
	lst = [1, 2, 3, 4, 5, 6]
	RMSE_Train_list = []
	RMSE_Test_list = []
	for iterator in regularised_weight:
		iteration = []
		RMSE = []
		RMSET =[]
		theta = np.zeros(5)
		for i in range(500):
			temp = 0
			#print("Theta",theta)
			theta0 = regularization(theta, X, y, m, learning_rate,iterator)
			improvement = theta - theta0
			if np.dot(improvement,improvement) <= 0.00000001:
				temp = 1
			s = 0
		...
		for i in range(4323):	
			...
			s += (norm_min+(norm-norm_min)*hypothesis(X_test[i],theta)-y_test[i])**2
		sss = sqrt(s/4323)
		RMSE_Test_list.append(sss)
		#Testing RMSE :  348410.144422204 

	#plt.plot(lst , RMSE_Train_list , label="TRAIN" )
	plt.plot(lst , RMSE_Test_list,label = "TEST")
	plt.legend()
